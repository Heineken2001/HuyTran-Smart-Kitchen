<?php
    ...
    foreach ($all as $key => $value){
        ...
        // echo $time_now.", ";
        // echo $times3.", ";
        if($time_now == $times3 && $value['DevID'] == 7){
            $gas_data += $value['DATAS'];
            $gas_count += 1;
        }
        if($time_now == $times3 && $value['DevID'] == 4){
            $humid_data += $value['DATAS'];
            $humid_count += 1;
        }
        if($time_now == $times3 && $value['DevID'] == 5){
            $temp_data += $value['DATAS'];
            $temp_count += 1;
        }

        if($time_test2 == $times3 && $value['DevID'] == 2){
            ...
            <div class="row">
                ...
                <div class="col l-12 m-12 c-12 room__status__body__list" style="border-radius: 20px">
                    <div class="room__status__body__list__title" style="text-align: center;">
                        <input type="text" id="basicDate" placeholder="Please select Date Time" value="<?php $date_time?>" data-input>
                    ...
                    <div style="text-align: left; margin-top: 10px; margin-left: 30px; height: 50%; width: 50%; float:left">
                        ...
                        <br>
                        <h5 style="color: black; margin: 10px auto">Nồng độ Gas trung bình trong ngày <?php echo date("d-m-Y", time()+5*60*60);?> là: <?php echo $gas_average ?>ppm.</h5>
                        <br>
                        <h5 style="color: black; margin: 10px auto">Độ ẩm trung bình trong ngày <?php echo date("d-m-Y", time()+5*60*60);?> là: <?php echo $humid_average ?>%.</h5>
                        <br>
                        <h5 style="color: black; margin: 10px auto">Nhiệt độ trung bình trong ngày <?php echo date("d-m-Y", time()+5*60*60);?> là: <?php echo $temp_average ?>°C.</h5>
                        <br>
                        <h5 style="color: black; margin: 10px auto">Thời gian sử dụng đèn trong ngày:...</h5>
                        <br>
                        <h5 style="color: black; margin: 10px auto; margin-bottom: 50px">Số lần nồng độ Gas vượt ngưỡng trong ngày:...</h5>
                    </div>
